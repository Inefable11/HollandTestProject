  ...
  implements OnInit
{
  ...
  ngOnInit() {
    this.loadData();
    //console.log("dataSourceONInit:", this.teachersService.AllTeachers)
  }
  refresh() {
    this.loadData();
  }
  ...
  deleteItem(row) {
    ...
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {
      if (result === 1) {
        ...
        this.teachersService.deleteTeachers(row).subscribe(res=>{
          //console.log("res", res)          
          Swal.fire({
            position: 'center',
            icon: 'success',
            title: 'Éxito',
            text:'Se elimnó correctamente',
            showConfirmButton: true,          
          });
        this.loadData();
         })
  ...
  public loadData() {
    ...
    //console.log("dataSource:", this.dataSource.renderedData)
  }
