  ...
  implements OnInit
{
  ...
  editCall(row) {
    ...
    this.leaveRequestService.GetTestChart(this.id).subscribe(res=>{
      const dialogRef = this.dialog.open(FormDialogComponent, {
        data: {
          leaveRequest: row,
          chart: res,
...
export class ExampleDataSource extends DataSource<LeaveRequest> {
  ...
  connect(): Observable<LeaveRequest[]> {
    ...
    this.exampleDatabase.getAllLeaveRequests(currentUser.id);
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case 'section':
          ...
          break;
        case 'applyDate':
          [propertyA, propertyB] = [a.timeWork, b.timeWork];
          break;
        case 'fromDate':
